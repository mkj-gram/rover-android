function createRecords (records, segmentSize, page) {
    ...
    let firstNames = ['Alex', 'Maryan', 'Chris', 'Will', 'Sean', 'Mike', 'John', 'Steve']
    ...
    for (let i=0; i<records; i++) {
        devices.push({
            ...
            deviceOSVersion: deviceOSVersion[Math.floor(Math.random()*profileIds.length)],
            ...
            hardware: `iPhone ${i}`,
            model: `${i}: ${page}`,
            appVersion: appVersion[Math.floor(Math.random()*profileIds.length)],
            pushEnabled: true,
            bluetoothOn: true,
        ...
        profiles.push({
            profileId: profileIds[Math.floor(Math.random()*profileIds.length)],
            firstName: firstNames[Math.floor(Math.random()*profileIds.length)],
...
export const getSegmentPageByPredicates = (predicates, pageNumber, pageSize) => {
    let ret 

    if (predicates.length == 0 || (predicates['profiles'] && predicates['profiles'].length == 0 && predicates['devices'] &&predicates['devices'].length == 0)) {
        if (pageNumber===0) {
    ...
    } else if (predicates['devices'] && predicates['devices'].length > 0 && predicates['devices'][0]['stringValue'] == 'Android') {
