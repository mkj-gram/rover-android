...
open class ScreenViewController: UICollectionViewController, UICollectionViewDataSourcePrefetching {
    ...
    public init(collectionViewLayout: UICollectionViewLayout, experience: Experience, screen: Screen, dispatcher: Dispatcher, eventQueue: EventQueue?, imageStore: ImageStore, sessionController: SessionController) {
    ...
    override open func collectionView(_ collectionView: UICollectionView, shouldSelectItemAt indexPath: IndexPath) -> Bool {
        ...
        // No-op if the block does not have any actions assigned to it to avoid every rectangle, line, image etc. tracking events
        return screen.rows[indexPath.section].blocks[indexPath.row].action != nil
    }
    ...
    override open func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let action = block.action {
            ...
            // TODO: Store the action attributes on the event
            
            if let operation = dispatcher.dispatch(action) as? GoToScreenOperation {
                operation.experience = experience
                operation.navigationController = navigationController
            }
        }
