...
open class NotificationCenterViewController: UIViewController {
    ...
    public init(dispatcher: Dispatcher, eventQueue: EventQueue?, imageStore: ImageStore, notificationStore: NotificationStore, sessionController: SessionController) {
...
extension NotificationCenterViewController: UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        return [UITableViewRowAction(style: .destructive, title: "Delete") { (action, indexPath) in
            self.markNotificationDeleted(indexPath: indexPath)
            }]
    }
    ...
    func markNotificationDeleted(indexPath: IndexPath) {
        let notification = notifications[indexPath.row]
...
extension NotificationCenterViewController: NotificationHandler {
    open func openNotification(_ notification: Notification) {
        ...
        if let action = notification.action, let operation = dispatcher.dispatch(action), let presentViewOperation = operation as? PresentViewOperation {
            presentViewOperation.viewControllerToPresent.transitioningDelegate = self
        }
