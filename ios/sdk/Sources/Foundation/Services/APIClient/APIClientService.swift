...
class APIClientService: APIClient {
    ...
    init(accountToken: String, endpoint: URL, session: HTTPSession) {
    ...
    func downloadTask<T>(with operation: T, completionHandler: @escaping (HTTPResult) -> Void) -> HTTPTask where T: GraphQLOperation {
    ...
    func uploadTask<T>(with operation: T, completionHandler: @escaping (HTTPResult) -> Void) -> HTTPTask where T: GraphQLOperation {
    ...
    /**
     * Convert a response from a URLSessionTask completionHandler into an HTTPResult.
     */
    func graphQLResult(from data: Data?, urlResponse: URLResponse?, error: Error?) -> HTTPResult {
        if let error = error {
        ...
        guard var data = data else {
            return .error(error: HTTPError.emptyResponseData, isRetryable: false)
        }
        ...
        if data.isGzipped {
            do {
                ...
                return .error(error: HTTPError.failedToUnzipResponseData, isRetryable: false)
            }
        }
