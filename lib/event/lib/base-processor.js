...
class BaseProcessor {
    ...
    filterMessages(messageTemplates, callback) {
        this.filterMessagesBySegment(messageTemplates, (err, filteredMessageTemplates) => {
            ...
            this.filterMessagesByRateLimits(filteredMessageTemplates, (err, filteredMessageTemplates) => {
                if (err) {
                    return callback(err);
                }
    ...
    filterMessagesByRateLimits(messageTemplates, callback) {
        ...
        Promise.map(messageTemplates, (messageTemplate) => new Promise((resolve, reject) => {
            ...
            let limits = (accountLimits.concat(messageTemplate.limits)).map(limit => internals.parseLimit(limit));


            if (limits.length == 0) {
                // don't do anything
                return resolve(messageTemplates);
            }


            // Sort the limits so that we are starting with the smallest limit to the largest
            // more than likely we are hitting the smallest limit first
            let sortedLimits = limits.sort((a,b) => { return a.number_of_seconds >= b.number_of_seconds });
            let largestLimit = sortedLimits[sortedLimits.length - 1];
            let currentTime = moment.utc(new Date);

            let dropInterval = currentTime.subtract(largestLimit.number_of_seconds, 'seconds').unix();
            // first drop everything past the largest limit
            // this is for cleanup purposes
            
            let key = RATE_LIMIT_KEY_PREFIX + this._customer._id.toString(); 
            
            let tasks = [];

            tasks.push((callback) => {
                // value looks like { August 8th 10:00pm, August 9th 5:12pm, ...}
                // if dropInterval is August 8th 11:00pm then we get { August 9th 5:12pm, ...}
                redis.zremrangebyscore(key, "-inf", dropInterval, (err, response) => {
                    if (err) {
                        return callback(err);
                    }
            ...
            sortedLimits.forEach(limit => {
                tasks.push((callback) => {
                    ...
                    redis.zcount(key, limitInterval, "+inf", (err, count) => {
                        if (err) {
                            return callback(err);
                        }
    ...
    deliverMessages(messageTemplates, callback) {
        ...
        let messages = messageTemplates.map((messageTemplate) => {
            let message = {
                ...
                landing_page: messageTemplate.landing_page,
        ...
        methods.message.bulkInsert(messages, (err, insertedMessages) => {
            ...
            Promise.all(messageEventPromises).then(() => {
