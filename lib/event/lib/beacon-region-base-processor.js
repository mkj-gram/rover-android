...
class BeaconRegionBaseProcessor extends BaseProcessor {
    ...
    getMessages(callback) {
        ...
        methods.proximityMessage.beaconTriggered(this._account.id, this._eventId, this._timestamp, this._beaconConfiguration, this._place, (err, messages) => {
            if (err) {
                return callback(err);
            }
        ...
        if (beaconConfiguration) {
            let beaconConfigurationRecord = {
                configuration: {
                    ...
                    shard: beaconConfiguration.shared,
    ...
    toResponse() {
     ...
     afterProcessed(callback) {
        ...
        // Only if the p
        if (place && place.radius <= 250 && util.isNullOrUndefined(device.location) || ( device.location && ( device.location.latitude != place.latitude || device.location.longitude != place.longitude ))) {
