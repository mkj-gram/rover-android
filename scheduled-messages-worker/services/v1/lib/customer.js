...
internals.findAll = function(ids, callback) {
    const server = this; 
    ...
    mongodb.collection(collection).find({ _id: { $in: ids }}).toArray((err, docs) => {
        if (err) {
...
internals.updateMany = function(ids, update, callback) {
    const server = this;
    const mongodb = server.connections.mongodb.client;
    const ObjectId = server.connections.mongodb.ObjectId;
    const logger = server.plugins.logger.logger;

    let customerIds = ids.map(id => {
    ...
    logger.debug('Service: [customer.updateMany] \n' + util.inspect(update, true, null, false));
    mongodb.collection(collection).updateMany({ "_id": { "$in": customerIds }}, update, function(err, response) {
        if (err) {
            return callback(err);
        }
...
module.exports = {
    ...
    updateMany: internals.updateMany,
