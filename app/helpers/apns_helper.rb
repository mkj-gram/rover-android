        ...
        def messages_to_notifications(messages_by_token)
            ...
            messages_by_token.each do |token, messages|
                messages.each do |message|
                    ...
                    notifications.push(ApnsKit::Notification.new(id: message.id, token: token, sound: 'default', alert: { title: message.ios_title, body: message.notification_text }, data: payload, _rover: true))
                end
            end
