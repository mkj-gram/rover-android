...
const versioned_experiences_collection = 'versioned_experiences';
...
internals.findVersion = function(id, args, callback) {
    const server = this;
    const mongodb = server.connections.mongodb.client;
    const ObjectId = server.connections.mongodb.ObjectId;
    const logger = server.plugins.logger.logger;

    logger.debug(`Service: [experience.findVersion: ${id}] ` + util.inspect(args, true, null, false));
    ...
    mongodb.collection(versioned_experiences_collection).findOne({ _id: ObjectId(id) }, formattedArgs, function(err, docs) {
...
module.exports = {
    find: internals.find,
    findVersion: internals.findVersion
}